...
p <- eventReactive(input$run, {
  ...
  if (good == 3) { # if have data on that route during that time
    ...
    myPlot <- 
      ...
      # geom_bar(aes(x="", y=DELAYED_BOOL), stat="identity", width = 1) +
      ...
      # ggplot(dat2plot, aes(y=DEP_DELAY)) +
      # geom_bar() +
    ...
    myPlot <- ggplot() +
      annotate("text", x = 4, y = 25, size = 8, label='No Data Available at Your Departure Time') +
      theme_void()
  }
